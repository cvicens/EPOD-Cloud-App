var $fh = require('fh-mbaas-api');

function create (collection, object, ts) {
  console.log('db-store: create', collection, JSON.stringify(object), ts);
  if (typeof object === 'undefined' || typeof object.id === 'undefined' || object.id == '') {
    console.log('db-store: create ERROR', 'wrong data object cannot be empty and object.id.length > 0');
  }

  // Create
  var options = {
    "act": "create",
    "type": collection,
    "fields": {
      "id": object.id,
      "payload": object
    }
  };
  if (ts) {
    options.ts = ts;
  }
  $fh.db(options, function (err, data) {
    if (err) {
      console.log("db-store: create ERROR ", err);
    } else {
      console.log("db-store: create data", JSON.stringify(data));
    }
  });
}

function read (collection, id) {
  console.log('db-store: read function not implemented yet!!!');
}

function update (collection, object) {
  console.log('db-store: update', collection, JSON.stringify(object));
  setTimeout(function () {
    var options = {
       "act": "list",
       "type": collection,
       "eq": {
         "id": object.id
       }
     };
     $fh.db(options, function(err, data){
       if (err){
         console.log("db-store: update (list) ERROR ", err);
       } else {
         if (data.count <= 0) {
           // Create
           var options = {
             "act": "create",
             "type": collection,
             "fields": {
               "id": object.id,
               "payload": object
             }
           };
           $fh.db(options, function (err, data) {
             if (err) {
               console.log("db-store: update (create) ERROR ", err);
             } else {
               console.log("db-store: update (create) data OK");
             }
           });
         } else {
           // Update
           var _item = data.list[0];
           var guid = _item.guid;
           options = {
             "act": "update",
             "type": collection,
             "guid": guid,
             "fields": {
               "id": object.id,
               "payload": object
             }
           };
           $fh.db(options, function (err, data) {
             if (err) {
               console.log("db-store: update (update) ERROR ", err);
             } else {
               console.log("db-store: update (update) data", JSON.stringify(data));
             }
           });
         }
       }
     });
  }, 0);
}

//function delete (collection, object) {
//  console.log('db-store: delete', collection, JSON.stringify(object), ts);
//}

function list (collection, filter) {
  console.log('db-store: list function not implemented yet!!!');
}


function updateOld (customerid, device) {
  console.log('last-time for ', customerid, device);

  // We add a delay to avoid reading the updated time :-)
  setTimeout(function () {
    // Update/Patch a single field
    var options = {
       "act": "list",
       "type": "profiles",
       "eq": {
         "customerid": customerid
       }
     };
     $fh.db(options, function(err, data){
       if (err){
         console.log("ERROR ", err);
       } else {
         if (data.count <= 0) {
           console.log("updateLastTime->customerid: ", customerid, 'not found');
         } else {
           var profile = data.list[0];
           var guid = profile.guid;
           var fields = profile.fields;
           fields.lastActiveTime = moment().tz("Europe/Madrid").format();
           fields.lastActiveDevice = device;

           options = {
             "act": "update",
             "type": "profiles",
             "guid": guid,
             "fields": fields
           };
           $fh.db(options, function (err, data) {
             if (err) {
               console.error("Error " + err);
             } else {
               console.log(JSON.stringify(data));
             }
           });
         }
       }
     });
  }, 5000);
}

module.exports.create = create;
module.exports.read = read;
module.exports.update = update;
//module.exports.delete = delete;
module.exports.list = list;
